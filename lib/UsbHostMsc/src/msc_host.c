...
esp_err_t msc_host_install_device(uint8_t device_address, msc_host_device_handle_t *msc_device_handle)
{
    ...
    uint8_t max_lun = 0;
    esp_err_t lun_err = msc_get_max_lun(msc_device, &max_lun);
    if (lun_err != ESP_OK) {
        max_lun = 0;
    }
    max_lun = MIN(max_lun, 15);
    ESP_LOGI(TAG, "Device reports max LUN %u (query=%d)", max_lun, (int)lun_err);

    ret = ESP_ERR_NOT_FOUND;
    for (uint8_t lun = 0; lun <= max_lun; ++lun) {
        msc_device->lun = lun;
        ESP_LOGI(TAG, "Probing LUN %u", lun);
        if (scsi_cmd_inquiry(msc_device) != ESP_OK) {
            continue;
        }
        if (msc_wait_for_ready_state(msc_device, 1500) != ESP_OK) {
            continue;
        }
        if (scsi_cmd_read_capacity(msc_device, &block_size, &block_count) != ESP_OK) {
            continue;
        }
        ret = ESP_OK;
        break;
